...
Tests KR920 per-channel duty cycle enforcement (2% per channel with LBT).

Sub-cases:
- DISABLED: duty_cycle_enabled: false - all frames pass
- SINGLE_CH: Single channel DC - 2% limit (stricter than AS923)
...
import logging
logger = logging.getLogger('test9c-kr920')
...
# KR920 has 2% per-channel DC (stricter than AS923's 10%)
# SF7/125kHz 6-byte payload ≈ 51ms airtime
# 2% DC: 51ms * 50 = 2550ms off-time per channel
...
TEST_CASES = {
    ...
    'SINGLE_CH': {
        ...
        'freqs': [(CH1, CH1)] * 5,
        'intervals': [2.0] * 5,  # 2s ~= 2.55s needed, may pass most
        'min_tx': 3, 'max_tx': 5,
        'desc': 'Single channel 2% DC - mostly passes with 2s intervals'
    },
    'MULTI_CH': {
        'duty_cycle_enabled': None,
        'freqs': [
...
async def run_test(test_name):
    ...
    logger.info('='*60)
    logger.info('KR920 Test: %s' % tc['desc'])
    logger.info('='*60)
    ...
    muxs = TestMuxs()
    muxs.router_config = tu.router_config_KR920
